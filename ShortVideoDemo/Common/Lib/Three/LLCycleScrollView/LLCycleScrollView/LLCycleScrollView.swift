...
open class LLCycleScrollView: UIView {
    ...
    open var spacing: UIEdgeInsets = UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0)
    ...
    var _placeHolderImage: String = ""
    open var placeHolderImage: String? = nil {
        didSet {
...
//    fileprivate var placeHolderViewImage: UIImage! = UIImage(named: "LLCycleScrollView.bundle/llplaceholder.png", in: Bundle(for: LLCycleScrollView.self), compatibleWith: nil)
    ...
    /// 空数据占位图
    fileprivate var coverViewImage: UIImage! = UIImage(named: "image_def_1")
...
extension LLCycleScrollView: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if isOnlyTitle && titles.count > 0{
            ...
            if let imageCorner = imageCorner {
                cell.imageCorner = imageCorner
            }
            // 0==count 占位图
            if imagePaths.count == 0 {
                ...
                if imagePath.hasPrefix("http") {
                    cell.imageView.rbSetImage(urlStr: imagePath, defaultImageName: defauleNoImage)
                }else{
                    if let image = UIImage.init(named: imagePath) {
                        cell.imageView.image = image;
                    }else{
                        let i = UIImage.init(contentsOfFile: imagePath)
                        if i != nil {
...
//        let _: LLCycleScrollViewCell = collectionView.dequeueReusableCell(withReuseIdentifier: identifier, for: indexPath) as! LLCycleScrollViewCell
